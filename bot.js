    ...
    if( ( newMember.roles.has( config.trialRole ) && ! oldMember.roles.has( config.trialRole ) ) || addTrial === true ) {
        ...
        if ( trialEligible === true ) {
            ...
            if( freeTrialSet ) { 
                ...
                // send removal notice
                bot.channels.get( config.upgradeChannel ).send( newMember + ", A free trial has been added to your account." );
            } else {
                // Add the role!
                ...
                bot.channels.get( config.upgradeChannel ).send( newMember + ", Looks like you already had a free trial and cannot add another." ); 
            }
        } else {
            // remove trial
            ...
            // send removal notice
            bot.channels.get( config.upgradeChannel ).send( newMember + ", Looks like you already had a trial. This command cannot be used to add another one." );
        }
    }
